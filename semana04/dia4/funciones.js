// que es una funcion?
//una funcion es un conjunto
// de soluciones para poder realizar tareas

function nombreDeLaFuncion() {
  //una vez declarada la funcion todo el codigo que haga dentro de esta solo
  //va a ejecutarse cuando se llame a la funcion

  console.log("mi primera funcion");
  alert("mi primera funcion");
}

//nombreDeLaFuncion();

function sumar(numero1, numero2) {
  const suma = numero1 + numero2;
  console.log("Resultado es: ", suma);
}
sumar(100, 20);

//tipos de funciones
//existen 3 tipos de funciones: declarativas, expresivas o anonimas
//y arrow o funcion flecha

//anteponemos la palabra function

function suma(a, b, c) {
  return a + b + c;
}

console.log(suma(10, 10, 10));

//funcion anonima o expresiva

let suma2 = function (a, b) {
  return a + b;
};
console.log(suma2(20, 10));
//arrow function
const suma3 = (x, y) => {
  return x + y;
};
console.log(suma3(20, 10));

//funcion saludar al alumno
const saludar = (nombre, apellido) => {
  // return "hola mi nombre es " + nombre + " y mi apellido es " + apellido;
  // template string
  return `Hola mi nombre es ${nombre} y mi apellido es ${apellido}`;
};
console.log(saludar("Jesus Daniel", "Chiroque Martinez"));
//--------------------------------------------------

const mostrarNombre = () => {
  let elementoNombre = document.getElementById("nombreUsuario");
  let title = document.getElementById("reemplazo");
  let mensaje = "Tú te llamas " + elementoNombre.value;

  title.textContent = mensaje;
};
//--------------------------------------------------

const mostrarTexto = () => {
  let elementoTexto = document.getElementById("inputTexto");
  let title = document.getElementById("titulo");
  title.textContent = elementoTexto.value;
};
//-----------------------ejercicio 2---------------------------

const mostrarNombre2 = () => {
  let elementoNombre2 = document.getElementById("inputTexto2");
  let title2 = document.getElementById("titulo2");
  let mensaje2 = "Mi artista favorito es: " + elementoNombre2.value;

  title2.textContent = mensaje2;
};
//----------------------- ejemplos ---------------------------
const comenzarAlarma = () => {
  setTimeout(tiempoCumplido, 1000 * 3);
};
const tiempoCumplido = () => {
  alert("Se acabó el tiempo");
};
//---------------------ejemplo 2---------------
let elementoSegundos = document.getElementById("tiempoElegido");
let elementoTextoAlarma = document.getElementById("textoAlarma");
const comenzarTiempo = () => {
  setTimeout(tiempoCumplido2, 1000 * elementoSegundos.value);
};
const tiempoCumplido2 = () => {
  elementoTextoAlarma.textContent = "Encendido";
  elementoTextoAlarma.style.color = "green";
};
